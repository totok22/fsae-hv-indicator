...
4. **视觉指示与微光消除 (Anti-Ghosting)：** 
   由 4 颗串联的 AlInGaP 高亮红色 LED (HSMZ-A100-T70J1) 与并联泄放电阻 R1 (100kΩ) 组成。多颗串联实现在微电流下扩大发光面积；R1 用于旁路 MOSFET 关断状态下的漏电流（$`I_{\mathrm{DSS}}`$），防止指示灯在低于阈值电压时出现不合规的微弱发光。
...
*   **设定点：** 查阅 UTC 3N100-C 规格书，在极小电流（如 $250\mu\text{A}$ ）下，$V_{GS(th)}$ 典型值为 **4.0V**。
*   **控制支路电流：** 当 $V_G = 4.0\text{V}$ 时，流经下拉电阻 R6 (2MΩ) 的电流 $I_{div}$ 为：
...
*   **电流与发热：** 替换后，稳态恒流阶段由 R7 设定的恒流 $I_D$ 会略微增大（约 $0.88\text{mA}$ ）。由于 TO-220 封装的结到环境热阻 $\theta_{JA}$（$`62^\circ\mathrm{C/W}`$）远低于 TO-252（$`110^\circ\mathrm{C/W}`$），自身温升将从约 $51.7^\circ\text{C}$ 显著降至约 $32.2^\circ\text{C}$。
